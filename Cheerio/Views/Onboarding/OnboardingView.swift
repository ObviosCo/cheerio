...
///
/// Shown automatically once — `ContentView` hands off to this window itself, from
/// its own `onAppear`, for as long as ``OnboardingState/hasCompleted`` is false (see
/// #63: `.defaultLaunchBehavior` alone doesn't reliably keep the main window from
/// claiming launch instead on macOS 26) — and re-openable afterward from Settings
/// and the Help menu. Recording is never gated on any of this — every screen can be
/// skipped, and `CaptureSession.start` doesn't know this window exists.
