...
3. **Pick the roster per meeting, not the oldest four.** Sortformer's 4-speaker cap is a hard ceiling, but `SpeakerLabeling.enrollments()` currently spends it on `prefix(4)` by enrolment date — so once five people are enrolled, someone who *was* in the room silently loses their slot to someone who wasn't. Wanted instead: keep as many saved voices as you like, and choose which are in *this* meeting (you're always one of them). A Jackson/Sarah/Whitney meeting primes three voices, not four arbitrary ones. Note enrolment only helps voices sharing a channel — in the room, or coming down one remote feed; an all-remote call needs none, since the mic/system split already separates you from them. Retroactive enrolment (task now done, see `MeetingSpeakersSection`) makes this urgent: collecting voices is cheap now, so hitting the cap is a matter of time.
