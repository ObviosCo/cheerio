...
Three images. `social-card.png` is committed and real — generated by
`Scripts/render-wordmark.swift`. The two `library*.png` files are committed
**placeholders** (see the last section); the site expects real screenshots in
their place before it ships.
