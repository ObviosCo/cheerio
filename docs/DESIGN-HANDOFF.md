...
| DMG / download presentation | See §8 | Direct distribution | **Shipped, unpolished** (#55) — working layout, art is placeholder |
...
| Live recording | [RecordingSurface.swift](../Cheerio/Views/RecordingSurface.swift) (session wiring in [RecordingView.swift](../Cheerio/Views/RecordingView.swift)) | Editable title + roster menu + timer header; `VSplitView` with notes on top (ideal 460pt) and transcript below (ideal 180pt). The layout takes plain values, so the audits and the screenshot harness can reach it without a recording (#164) |
...
6. **The meeting detail view is a long vertical stack.** It's had a first information-architecture pass since this was written (issues #104/#109): the transcript — still the one section that can run to hundreds of lines — now collapses by default instead of opening expanded, every section (notes, rough notes, the re-identify row, speakers, transcript) is separated by a rule rather than sharing one undifferentiated stack, the title renders once with a visible rename affordance instead of twice with none, and rough notes are editable again after the meeting rather than frozen. What that pass didn't attempt is a real hierarchy: everything below the title is still one altitude of "section," just with clearer edges between them. Whether the primary read, the reference material, and the tools deserve visually distinct treatment — not just a divider apiece — is still open.
...
The DMG now exists and ships with every release (#55) — it had to, ahead of any design pass, because the zip was actively breaking self-updates: people ran `Cheerio.app` out of `~/Downloads`, macOS translocated it, and Sparkle could never update it again. **What's built is the mechanism, not the art.** The window is 640×400 with the app on the left, `/Applications` on the right, and a copper arrow between them, over a navy gradient with two icon wells and two lines of caption text. It is tidy and it is generic.

What's yours to take, if you want it:

- The background art itself. It is generated by `Scripts/render-dmg-background.swift` (CoreGraphics, 1x + 2x, same convention as the app icon) rather than drawn, so a redesign means editing that script, not handing over a PNG. Geometry lives in two places that must agree — the script's well centres and `Scripts/dmg-settings.py`'s `icon_locations` — and `Scripts/verify-dmg.py` fails the build if the result doesn't match the settings. Change both together.
- The wordmark is deliberately absent. There was no strong reason to add it and no layout worked out for it; if the window should carry the mark, that's a design call.
- The two caption lines are functional, not styled: "Drag Cheerio to your Applications folder" and, quieter, "Already installed? Cheerio updates itself." The second one exists to head off a dead end — someone with Cheerio already running drags anyway and Finder refuses with "Cheerio can't be replaced because it's open," which reads like a failure and suggests no way out.
- Volume icon: none set. A DMG can carry its own.
