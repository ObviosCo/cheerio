...
- **`BundleIdentifierMigration`** renames the whole `~/Library/Application Support/app.cheerio.mac` directory to `co.obvios.cheerio.mac` in one same-volume `FileManager.moveItem` — atomic, and it either lands whole or not at all. `CheerioApp.init` calls it before constructing the `ModelConfiguration`, ahead of `StorageMigration` (which is unrelated — see above — and runs later, once a `ModelContext` exists). Idempotent: a directory that's already moved, or was never there (a fresh install), is a no-op; a destination that already exists is left alone rather than clobbered, on the chance it holds data written by a launch that already migrated. On a genuine move failure (permissions, most plausibly) the old directory is left in place and `AudioStorage.setContainerOverride(_:)` points the *whole* process — not just the store, but every meeting-audio and voice-sample path resolved through `AudioStorage.applicationSupport()` — at the old identifier for that launch, rather than open an empty new container and present that as the library. The override is a `Mutex`-protected global rather than a parameter threaded through every audio call site, written exactly once before anything concurrent could read it. Nothing prevents two Cheerio processes launching at once, so the existence checks and the `moveItem` are not one atomic step from the caller's point of view — a second launch can win the identical rename in between. `migrate`'s `catch` block re-checks both paths before concluding failure: if the source is gone and the destination now holds the migrated container, that's another launch's success, not this one's failure, and it's reported as `.migrated` rather than triggering the override on a container that no longer exists.
