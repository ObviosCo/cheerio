...
**None of this runs for a fork.** `BundleIdentifierMigration`, `UserDefaultsMigration`, and the DMG handoff's legacy match are all gated behind `AudioStorage.isRunningAsCanonicalIdentifier()` — true only when the running process's identifier equals `AudioStorage.appBundleIdentifier`. A fork that changes `PRODUCT_BUNDLE_IDENTIFIER` in `project.yml` without also changing `appBundleIdentifier`'s value would otherwise inherit all three unintentionally: its DMG build could find an unrelated, independently-installed `app.cheerio.mac` copy, treat it as itself already installed, hand off to that unrelated app, and quit; and — had it happened to also ship under `app.cheerio.mac` at some point — its launches would migrate and read *Cheerio's* legacy data, not its own. Changing `appBundleIdentifier` is therefore the second required edit alongside `project.yml`, documented in README.md's "Building your own fork" bullet; nothing here derives one from the other.
