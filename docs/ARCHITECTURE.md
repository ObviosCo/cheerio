...
**Sparkle and the identifier change — can an existing `app.cheerio.mac` install auto-update to a `co.obvios.cheerio.mac` build at all?** Yes. Sparkle 2.9.5's installer (`Autoupdate/SUInstaller.m:35`, in the pinned SwiftPM checkout) locates the new bundle inside the downloaded archive by matching the *filename* — `[[host bundlePath] lastPathComponent]`, i.e. `Cheerio.app` — not the identifier; a bundle-identifier comparison exists only as a fallback (`SUInstaller.m:100-106`) for when the filename doesn't match, which it does here since the app isn't being renamed. The identifier check in `Autoupdate/AppInstaller.m:445-452` ("Failed to match host bundle identifiers") compares the *not-yet-replaced* old bundle's identifier against itself at two points in an IPC handshake — an integrity check that the installer is still talking about the app it was launched for, not a comparison against the new download's identifier, so it can't fail on an identifier change. And `SUAppcastItem` (`Sparkle/SUAppcastItem.m`) carries no bundle-identifier field at all, so nothing in appcast parsing gates on one either. Net effect: Sparkle overwrites `Cheerio.app` in place with the new bundle regardless of what `CFBundleIdentifier` that bundle carries — an existing install auto-updates across the rename with no manual DMG download required. What *does* change silently is which preferences domain the running process reads afterward — which is exactly what `UserDefaultsMigration` above exists to paper over — and which TCC permissions macOS considers already granted, which it doesn't (see Permissions & entitlements, below).
