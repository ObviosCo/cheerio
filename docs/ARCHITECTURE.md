...
- **`BundleIdentifierMigration`** moves the whole `~/Library/Application Support/app.cheerio.mac` directory's contents to `co.obvios.cheerio.mac`, preferring one same-volume `FileManager.moveItem` on the whole directory — atomic, landing whole or not at all — whenever the new location doesn't already exist. `CheerioApp.init` calls it before constructing the `ModelConfiguration`, ahead of `StorageMigration` (which is unrelated — see above — and runs later, once a `ModelContext` exists). The both-exist verdict (`.bothExist`) keys on a *store* existing at the new location (`default.store`), not on the new directory merely being present: issue #126 was a real user's whole library going missing because the MCP helper's `MeetingStore.resolveStoreURL` had resolved — and, at the time, that resolution's own filesystem calls left behind — an empty `co.obvios.cheerio.mac` directory before the rebuilt app ever launched, and the old guard read that bare directory as "already migrated" and walked away, stranding an 18-meeting library under the old identifier. A new directory with no store in it now gets the old container's contents merged in item by item instead — same idempotence and never-clobber guarantees, just per-item rather than per-directory: an item whose name already exists at the destination is left where it is rather than overwritten, and the old directory is removed only once nothing remains in it. `MeetingStore.resolveStoreURL` and `AudioStorage.containerURL` are otherwise pure — they resolve a path without creating anything along the way — precisely because this scenario showed what a path-resolution function that *does* create something can strand upstream of it; that invariant is unit-tested at both call sites now, not just documented. On a genuine move failure (permissions, most plausibly — or an item-by-item merge that stops partway through) the old directory is left in place (partially emptied, if a merge got partway) and `AudioStorage.setContainerOverride(_:)` points the *whole* process — not just the store, but every meeting-audio and voice-sample path resolved through `AudioStorage.applicationSupport()` — at the old identifier for that launch, rather than open an empty or partially-filled new container and present that as the library. The override is a `Mutex`-protected global rather than a parameter threaded through every audio call site, written exactly once before anything concurrent could read it. Nothing prevents two Cheerio processes launching at once, so the existence checks and the move itself are not one atomic step from the caller's point of view — a second launch can win the identical migration in between. `migrate`'s `catch` block re-checks both paths before concluding failure: if the source is gone and the destination now holds a store, that's another launch's success, not this one's failure, and it's reported as `.migrated` rather than triggering the override on a container that no longer exists. (The maintainer's actual stranding — both containers ending up with a store, the new one with the one meeting recorded since — needed a one-time manual merge; there's no in-app recovery for two already-populated stores, only for the bare-directory case above.)
